...
export const OpenAISettings = ( { pluginSettings } ) => {
	...
	const fixMaxTokens = gptModel.includes('gpt-4.1') && (currentMaxTokens <= 30767 || currentMaxTokens > 32768)
		? 32768
		: gptModel.includes('gpt-4o') && (currentMaxTokens <= 14383 || currentMaxTokens > 16384)
			? 16384
			: gptModel === 'o1-mini' && (currentMaxTokens <= 63535 || currentMaxTokens > 65536)
				? 65536
				: gptModel === 'o3-mini' && (currentMaxTokens <= 97999 || currentMaxTokens > 100000)
					? 100000
					: currentMaxTokens;
	...
	return (
		<>
			...
			>
				...
				<VStack spacing={ 3 } className='dpaa-ai-assistant--settings__components-flex __open-ai'>
					<VStack spacing={ 1 }>
						...
						<CustomSelectControl
							...
							label={ <>
								...
								<PopoverHelp
									...
									help={ sprintf( __( 'Select the %s language model to generate text.', dpaa.i18n ), 'OpenAI' ) }
								/>
								<Button
									variant="link"
									size="small"
							...
							help={ __( 'The language model to use for text generation.', dpaa.i18n ) }
						/>
						<Text size={ 12 } style={ { color: '#666' } }>
							{ OPEN_AI_GPT_MODELS.find( option => option.key === gptModel )?.__experimentalHint }
						</Text>
					</VStack>
